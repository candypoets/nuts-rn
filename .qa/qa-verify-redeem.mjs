...
if (!community?.relay_url) {
  console.error(
    `no QA community state at ${
      process.env.QA_STATE || '/tmp/qa-rn-community.json'
    }`,
  );
...
if (!user?.pub)
  throw new Error(
    'keys file has no users[] (the device flow logs in as users[0])',
  );
...
async function main() {
  ...
  assert(
    issuer !== keys.admin.pub,
    'badge issuer is the service key, not the admin',
  );

  // 1. kind 8 award for the redeemer.
  const awards = await queryUntil({
    kinds: [8],
    authors: [issuer],
    '#p': [user.pub],
  });
  ...
  assert(
    award,
    'kind 8 badge award for the redeemer exists on the community relay',
  );
  assert(
    award.pubkey === issuer,
    'award authored by the community badge_issuer_pubkey',
  );
  if (relayView.required_badge) {
    assert(
      award.tags.some(
        tag => tag[0] === 'a' && tag[1] === relayView.required_badge,
      ),
  ...
  const profiles = await queryUntil({
    kinds: [0],
    authors: [user.pub],
    limit: 5,
  });
  ...
  const [, definitionAuthor, ...definitionDParts] = (
    relayView.required_badge || ''
  ).split(':');
  ...
  assert(
    definition.tags.some(
      tag => tag[0] === 'permission' && tag[1] === '0' && tag[2] === 'write',
    ),
    'membership grants the NIP-97 kind-0 profile capability',
  );
  assert(
    definition.tags.some(
      tag => tag[0] === 'permission' && tag[1] === '1' && tag[2] === 'write',
    ),
    'membership grants the NIP-97 kind-1 posting capability',
  );
