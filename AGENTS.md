...
### Thread-focus anchoring: tried and reverted (2026-08-26)

The reply-thread screen briefly pinned the FOCUSED note while ancestor rows streamed in above it (commits `bcef593`→`ce6b3ff`: Feed `unwrappedMotionContent` + `maintainVisibleContentMinIndex` + a viewport-height bottom padding so `maintainVisibleContentPosition` had scroll range). It worked on Android but native MVCP never compensated on iOS (RN 0.86.0 `RCTScrollViewComponentView` prepare/adjust reads correct; root cause never isolated — needs an on-device diagnostic build). Rather than ship a JS-side iOS fallback, the whole mechanism was reverted; ancestors streaming in above simply push the focused row down again. If this is revisited: the wiring recap lives in the reverted commits, and upstream 0.87's #57294 only adds MVCP abort conditions, it does not fix under-compensation.

One gotcha from that work STILL applies (the fix stays in the code):
