...
export function StoreSub({name: nameParam, onClose, relay, visible}: StoreSubProps) {
  ...
  const openAward = useCallback(
    (awardId: string) => {
      if (!awardId) return;
      pushDistinct(router, {pathname: '/Award', params: {relay: normalizedRelay, award: awardId}});
    },
    [router, normalizedRelay],
  );
  ...
  // NIP-97 catalog: memberships are 30009; products and passes are 30402.
  useEffect(() => {
    ...
    const unsubscribe = subscribeToNostr(
      ...
      [
        {
          kinds: [ROLE_MEMBERSHIP_KIND, CLASSIFIED_LISTING_KIND],
          limit: 500,
          noCache: true,
          relays: [normalizedRelay],
          tags: {'#t': [...STORE_CATALOG_TOPICS]},
        },
      ],
      message => {
