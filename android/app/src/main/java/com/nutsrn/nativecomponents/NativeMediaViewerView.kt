...
class NativeMediaViewerView(context:Context):ViewGroup(context){
  ...
  private var sessionId="";private val defaultSession=UUID.randomUUID().toString();private var noteBytes:ByteArray?=null;private var relays=emptyList<String>();private var userPubkey="";private var reactionNonce=0
  ...
  fun setNoteBytes(v:ReadableArray?){noteBytes=v?.bytes()};fun setRelays(v:ReadableArray?){relays=v.strings()};fun setCurrentUserPubkey(v:String?){userPubkey=v.orEmpty()};fun setOptimisticReactionNonce(v:Int){reactionNonce=v}
